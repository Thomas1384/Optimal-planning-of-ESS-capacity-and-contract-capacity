...
class OnGrid_realtime_opt:
    ...
    def opt_result_record_data(self):
        ...
        self.ESS_DOD = np.array([[[[0.0 for i in range(96)] for netload_scenario in range(self.netload_scenario_nums)]for ASS_day_scenario in range(len(self.ASS_time))]for TOU_scenario in range(len(self.tou_uncer_prob))])
        self.Uess_cycle = np.array([[[[0.0 for i in range(96)] for netload_scenario in range(self.netload_scenario_nums)]for ASS_day_scenario in range(len(self.ASS_time))]for TOU_scenario in range(len(self.tou_uncer_prob))])
        self.Uess_c = np.array([[[[0.0 for i in range(96)] for netload_scenario in range(self.netload_scenario_nums)]for ASS_day_scenario in range(len(self.ASS_time))]for TOU_scenario in range(len(self.tou_uncer_prob))])
        self.Uess_dsc = np.array([[[[0.0 for i in range(96)] for netload_scenario in range(self.netload_scenario_nums)]for ASS_day_scenario in range(len(self.ASS_time))]for TOU_scenario in range(len(self.tou_uncer_prob))])
    ...
    def centralize_optimization(self):
        ...
        SoC_avg = np.array([[[[m.addVar(lb=0,vtype=GRB.CONTINUOUS)] for netload_scenario in range(self.netload_scenario_nums)]for ASS_day_scenario in range(len(self.ASS_time))]for TOU_scenario in range(len(self.tou_uncer_prob))])
        DoD_ESS = np.array([[[[m.addVar(lb=0,vtype=GRB.CONTINUOUS) for i in len_n] for netload_scenario in range(self.netload_scenario_nums)]for ASS_day_scenario in range(len(self.ASS_time))]for TOU_scenario in range(len(self.tou_uncer_prob))])
        ...
        # constraint
        # 計算儲能係數
        m.addConstr(EssSOC_c == self.Ess_efficiency/(self.time_scale), "ess_sizing_1")                                                  #ESS_c=det(t)*efficiency/Cap_ESS
        m.addConstr(EssSOC_dsc == 1/((self.time_scale)*self.Ess_efficiency), "ess_sizing_2")                                              #ESS_dsc=det(t)/(Cap_ESS*efficiency)
        ...
        for TOU_scenario in range(len(self.tou_uncer_prob)):
            for ASS_time_scenario in range(len(self.ASS_time)):
                for netload_scenario in range(self.netload_scenario_nums):     
                    for i in len_n:
                        if i == 0:
                            m.addConstr(SoC_ESS[TOU_scenario][ASS_time_scenario][netload_scenario][i] == self.ESS_SOCinitial * battery_cap*1000*Cap_ESS_scale + EssSOC_c * Pess_c[TOU_scenario][ASS_time_scenario][netload_scenario][i] - EssSOC_dsc * Pess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i], "ESS_soc_1")
                        else:
        ...
        # battery aging(放電->充電，計數一次循環)
        for TOU_scenario in range(len(self.tou_uncer_prob)):
            for ASS_time_scenario in range(len(self.ASS_time)):
                for netload_scenario in range(self.netload_scenario_nums):     
                    for i in len_n:
                        m.addConstr(Uess_cycle[TOU_scenario][ASS_time_scenario][netload_scenario][i] == Uess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i-1] * (1-Uess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i]))
                        # m.addConstr(Uess_cycle[TOU_scenario][ASS_time_scenario][netload_scenario][i] == Uess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i-1] * (Uess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i-1]-Uess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i]))
                        m.addConstr(DoD_ESS[TOU_scenario][ASS_time_scenario][netload_scenario][i] * battery_cap*1000*Cap_ESS_scale == Pess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i]/(self.time_scale*self.Ess_efficiency))
                    m.addConstr(SoC_avg[TOU_scenario][ASS_time_scenario][netload_scenario][0] == gp.quicksum(SoC_ESS[TOU_scenario][ASS_time_scenario][netload_scenario][i] for i in len_n)/(self.time_interval*self.time_scale))
            ...
            for ASS_time_scenario in range(len(self.ASS_time)):
                for netload_scenario in range(self.netload_scenario_nums): 
                    for i in len_n:
                        ...
                        self.ESS_SOC[TOU_scenario][ASS_time_scenario][netload_scenario][i] = SoC_ESS[TOU_scenario][ASS_time_scenario][netload_scenario][i].x / self.battery_cap / 1000
                        self.ESS_DOD[TOU_scenario][ASS_time_scenario][netload_scenario][i] = DoD_ESS[TOU_scenario][ASS_time_scenario][netload_scenario][i].x / self.battery_cap
                        self.Uess_cycle[TOU_scenario][ASS_time_scenario][netload_scenario][i] = Uess_cycle[TOU_scenario][ASS_time_scenario][netload_scenario][i].x
                        self.Uess_c[TOU_scenario][ASS_time_scenario][netload_scenario][i] = Uess_c[TOU_scenario][ASS_time_scenario][netload_scenario][i].x
                        self.Uess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i] = Uess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario][i].x
                        # grid
                ...
                for ASS_time_scenario in range(len(self.ASS_time)):
                    for netload_scenario in range(self.netload_scenario_nums):
                        ...
                        data_ = {'Pbuild': self.Pbuild[netload_scenario],
                                ...
                                'ESS DOD': self.ESS_DOD[TOU_scenario][ASS_time_scenario][netload_scenario],
                                ...
                                'Uess_c':self.Uess_c[TOU_scenario][ASS_time_scenario][netload_scenario],
                                'Uess_dsc':self.Uess_dsc[TOU_scenario][ASS_time_scenario][netload_scenario],
                                'Uess_cycle':self.Uess_cycle[TOU_scenario][ASS_time_scenario][netload_scenario]}
                ...
                for ASS_time_scenario in range(len(self.ASS_time)):
                    for netload_scenario in range(self.netload_scenario_nums):
                        ...
                        data = {'Case':[cnt],
                                ...
                                'ESS_cycle':[sum(self.Uess_cycle[TOU_scenario][ASS_time_scenario][netload_scenario])]}
